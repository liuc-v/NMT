...
def generate_masks(src, tgt, src_padding, tgt_padding, device, n_head):
    src_mask = (src != src_padding).unsqueeze(-2).to(device) # batch_size x 1 x src_len x 1
    tgt_mask = (tgt != tgt_padding).unsqueeze(-2).to(device) # batch_size x 1 x tgt_len x 1
    tgt_mask = tgt_mask & subseq_mask(tgt.size(-1)).to(device) # batch_size x 1 x tgt_len x tgt_len
    memory_mask = src_mask.unsqueeze(1) & tgt_mask.unsqueeze(-1) # batch_size x n_head x tgt_len x src_len
    return src_mask, tgt_mask, memory_mask

def subseq_mask(size):
    """创建下三角矩阵，用于生成解码器的掩码矩阵"""
...
class TransformerModel(nn.Module):
    ...
    def forward(self, src, tgt):
        # src_mask, tgt_mask, memory_mask = generate_masks(src, tgt, src_padding, trg_padding, self.device, self.n_head)
        # 对源语言序列进行嵌入和位置编码
        src = self.encoder_embedding(src) * math.sqrt(self.d_model)
        src = self.pos_encoder(src)
        # 对源语言序列进行Transformer编码
        src_mask = torch.triu(torch.ones(src.shape[1], src.shape[1]), diagonal=1).bool().to(self.device)
        ...
        tgt_mask = torch.triu(torch.ones(tgt.shape[1], tgt.shape[1]), diagonal=1).bool().to(self.device)
